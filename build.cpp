...
TEST_CASE("build family", "[build]") {
  ...
  SECTION("unary dynamic build") {
    auto built = kblib::build<std::vector<int>>(input.begin(), input.end(), unary_f);
    REQUIRE(equal(squared, built));
  }
  SECTION("binary dynamic build") {
    auto built = kblib::build<std::vector<int>>(input.begin(), input.end(), input2.begin(), binary_f);
  ...
  SECTION("binary array build") {
    auto built = kblib::build<arr>(input.begin(), input.end(), input2.begin(), binary_f);
    REQUIRE(equal(pminusone, built));
  }
  const arr iota = {0, 1, 2, 3, 4, 5, 6, 7};
  SECTION("dynamic generator build") {
    auto built = kblib::build<std::vector<int>>([x = 0]() mutable {return x++;}, 8);
